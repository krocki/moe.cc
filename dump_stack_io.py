...
# ---------------- helpers (kept consistent with your layer script) ----------------
...
@torch.no_grad()
def run_layers(model, x, L, theta=10000.0, causal=True):
  """Runs the first L decoder layers (no final norm/head)."""
  sd = model.state_dict()
  for l in range(L):
    # --- Norm1
    ...
    x_attn = attn_gqa_block(x1, Wq,bq, Wk,bk, Wv,bv, Wo,bo, qn,kn, causal, theta)
    x2 = x + x_attn  # residual
    ...
    x = x2 + moe_block(x3, gate_w, gate_b, experts, k=8)
  return x
...
def main():
  ...
  ap.add_argument("--ids-npy", type=str, default="", help="Optional .npy file with int32 token IDs [T]")
  args = ap.parse_args()
  ...
  if args.ids_npy:
    ids_np = np.load(args.ids_npy).astype(np.int64)
    assert ids_np.ndim == 1, "ids must be 1-D [T]"
    assert ids_np.shape[0] == args.seqlen, f"ids len {ids_np.shape[0]} != seqlen {args.seqlen}"
    ids = torch.from_numpy(ids_np)
  else:
    ids = torch.randint(low=0, high=V, size=(args.seqlen,), dtype=torch.long)

  # ----- 2) Embedding -> x ----------------------------------------------------
  emb_w = sd["model.embed_tokens.weight"].float()      # [V, D]
  D = emb_w.shape[1]
  x = emb_w[ids]                                       # [T, D]
  np.save(args.outbase + ".ids.npy", ids.numpy().astype(np.int32))
  np.save(args.outbase + ".x.npy",   x.numpy().astype(np.float32))

  # ----- 3) First L layers (no final norm/head) ------------------------------
  y = run_layers(model, x, args.layers, theta=args.rope_theta, causal=bool(args.causal))
  ...
  if w_final is None:
    raise RuntimeError("Could not find final norm weight (model.norm.weight / model.final_layernorm.weight).")
  w_final = w_final.float()

  # LM head is usually tied; fall back to embed if needed
  lm_w = sd.get("lm_head.weight")
  if lm_w is None:
    lm_w = emb_w
  else:
    lm_w = lm_w.float()

  y_norm = rmsnorm(y, w_final, 1e-6)           # [T, D]
  logits = y_norm @ lm_w.t()                   # [T, V]
  np.save(args.outbase + ".logits.npy", logits.numpy().astype(np.float32))

  print(f"Saved {args.outbase}.ids/x/y/logits.npy (T={args.seqlen}, D={D}, V={V}, L={args.layers})")
